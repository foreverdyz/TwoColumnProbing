...
function prob_order(
        con_matrix::AbstractSparseArray, clq_table::Vector{Vector{Int64}}, clq_length::Vector{Int64}, 
        c_mt::AbstractSparseArray, binary_length::Int64, bin_to_org::Dict{Int64, Int64}, 
        cand_number::Int64, max_probe_number::Int64
    )
    let
        local I, J, K = findnz(c_mt)
        local n = cand_number
        if length(I) > n
        ...
        #for x_i, var_con_len[i] is how many constraints containing i
        ...
        for i in 1:length(I)
            ...
            local x = I[i]
            local y = J[i]
            (_have_conflict(clq_table[x], clq_table[y]) > 0) && (conflict = 2)
            K[i] = ceil(Int64, (10*K[i] + clq_length[x] + clq_length[y])/conflict)
        end
        local m = min(length(I), max_probe_number)
        local counter_order = partialsortperm(K, rev = true, 1:m)#this bound also can be cahnged
