#build_model_parallel.jl

using JuMP
using SparseArrays

function build_model_parallel(
    ...
    con_ub, con_lb, _, _, 
    var_name, var_type, 
    obj_coef, obj_constant, is_min = model_info("mipdata/"*filename*".mps.gz");
    con_num = length(con_ub);
    con_type = zeros(con_num);
    var_num = length(var_ub);
    I, J, K = findnz(con_matrix);
    #transpose A
    con_trans = sparse(J, I, K, size(con_matrix)[2], size(con_matrix)[1]);
    ...
    for j in 1:con_num
        if con_ub[j] == con_lb[j]
            @constraint(model, sum(con_matrix[j, i] * x[i] for i in findnz(con_trans[:, j])[1]) == con_ub[j])
        else
            if con_ub[j] < Inf
                @constraint(model, sum(con_matrix[j, i] * x[i] for i in findnz(con_trans[:, j])[1]) <= con_ub[j])
            end
            if con_lb[j] > -Inf
                @constraint(model, sum(con_matrix[j, i] * x[i] for i in findnz(con_trans[:, j])[1]) >= con_lb[j])
            end
        end
    end

    for id in 1:threadnum
        if length(new_conflict_g[id]) > 0
            for rd in new_conflict_g[id]
        ...
        if length(aggre_g[id]) > 0
            for i in findnz(aggre_list_g[id])[1]
    ...
    con_matrix, con_trans, I, J, K = nothing, nothing, nothing, nothing, nothing;
    return model
end
