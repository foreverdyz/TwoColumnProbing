...
for i in st_term:en_term
    ...
    #Reduced Model
    presolve_info = readdlm("presolved_res/presolve_res_1.csv", ',');
    if presolve_info[i, 2]>0
    presolve_time = presolve_info[i, 2];
    model_reduced = file_model = read_from_file("presolved_res/presolved_data_1/"*filename);
    ...
    #Reduced Model
    # presolve_info = readdlm("presolved_res/presolve_res_2.csv", ',');
    # if presolve_info[i, 2]>0
    # presolve_time = presolve_info[i, 2];
    # model_reduced = read_from_file("presolved_res/presolved_data_2/"*filename);
    # set_optimizer(model_reduced, SCIP.Optimizer);
    # #use the following command for some tests disabling the default probing
    # #set_attribute(model_reduced, "propagating/probing/maxuseless", false);
    # set_attribute(model_reduced, "parallel/maxnthreads", 1);
    ...
    #Reduced Model
    presolve_info = readdlm("presolved_res/presolve_res_4.csv", ',');
    if presolve_info[i, 2]>0
    presolve_time = presolve_info[i, 2];
    model_reduced = read_from_file("presolved_res/presolved_data_4/"*filename);
    set_optimizer(model_reduced, SCIP.Optimizer);
    #use the following command for some tests disabling the default probing
    #set_attribute(model_reduced, "propagating/probing/maxuseless", false);
    set_attribute(model_reduced, "parallel/maxnthreads", 1);
    ...
    #Reduced Model
    # presolve_info = readdlm("presolved_res/presolve_res_8.csv", ',');
    # if presolve_info[i, 2]>0
    # presolve_time = presolve_info[i, 2];
    # model_reduced = read_from_file("presolved_res/presolved_data_8/"*filename);
    # set_optimizer(model_reduced, SCIP.Optimizer);
    # #use the following command for some tests disabling the default probing
    # #set_attribute(model_reduced, "propagating/probing/maxuseless", false);
    # set_attribute(model_reduced, "parallel/maxnthreads", 1);
    ...
    open(path1*"_1_16.csv", "a") do file
        # Write the data
        writedlm(file, [rd], ',')
    end
